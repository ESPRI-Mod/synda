...
def compute_latest_flag(dataset_versions,d):
    ...
    if dataset_versions.count()==0:
        ...
        if d.status==DATASET_STRUCTURE["status"]["empty"]:
            # do not contains any complete variable yet

            l__latest=False

        elif d.status==DATASET_STRUCTURE["status"]["in_progress"]:
...
def compute_dataset_status(d):
    ...
    total_done_files_count=sdfilequery.count_dataset_files(d,TRANSFER["status"]['done'])
    ...
    if total_done_files_count==0:
        l__status=DATASET_STRUCTURE["status"]["empty"]
    else:
        # TODO
        # some special status should not be considered done: TRANSFER["status"]['delete']

        if total_files_count == total_done_files_count:
            l__status=DATASET_STRUCTURE["status"]["complete"]
        else:
            ...
            if sdvariable.exists_one_complete_variable(d):
                ...
                l__status=DATASET_STRUCTURE["status"]["in_progress"]
            else:
                l__status=DATASET_STRUCTURE["status"]["empty"]
...
def update_incomplete_datasets_status():
    ...
    incomplete_datasets = \
        sddatasetdao.get_datasets(
            status=DATASET_STRUCTURE["status"]["empty"],
        ) + sddatasetdao.get_datasets(
            status=DATASET_STRUCTURE["status"]["in_progress"],
        )
...
def update_complete_datasets_status():
    ...
    complete_datasets=sddatasetdao.get_datasets(status=DATASET_STRUCTURE["status"]["complete"])
