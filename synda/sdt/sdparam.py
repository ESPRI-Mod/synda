...
def get_parameter_type(name):
    ...
    if values_count == 1:
        ...
        if values[0] is None:
            ...
            # raise SDException(
            #     "SYDPARAM-001",
            #     "Incorrect predefined value for '%s' parameter "
...
def get_name_from_value(value, show_info_message=False):
    ...
    if len(names)==0:
        ...
        if show_info_message:
            sdtools.print_stdout("INFO: '%s' value has been associated with '%s' facet."%(value,names[0]))
        return names[0]
