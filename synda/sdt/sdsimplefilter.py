...
def remove_matching_files_substr(files,filter_name,filter_value):
    ...
    for f in files:
        ...
        if filter_name not in f:
            raise SDException("SDSIMPLF-004","Filter name not found in file attributes (filter_name=%s)"%(filter_name,))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s','--status',choices=TRANSFER["statuses"]['all'],required=True)
