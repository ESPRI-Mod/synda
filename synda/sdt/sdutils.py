...
def compute_checksum(file_fullpath,checksum_type=CHECKSUM['type']["md5"],blocksize=(1024*64)):

    if checksum_type not in CHECKSUM['types']:
    ...
    with open(file_fullpath, mode='rb') as f:
        ...
        d=None
        if checksum_type==CHECKSUM['type']["md5"]:
            d = hashlib.md5()
        elif checksum_type==CHECKSUM['type']["sha256"]:
            d = hashlib.sha256()
        else:
            assert False
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t','--checksum_type',default=CHECKSUM['type']["sha256"])
