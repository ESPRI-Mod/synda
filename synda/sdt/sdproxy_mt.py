...
class MetadataThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            url_with_host_set = self.query['url'].replace(
                Internal().api_esgf_search_domain_name,
                self.host,
            )
...
def run(i__queries):
    ...
    # check
    for q in i__queries:
        if Internal().api_esgf_search_domain_name not in q['url']:
            raise sdexception.SDException(
                'SDPROXMT-044',
                'Incorrect query: host must not be set at this step',
            )
...
def run_helper(queries):
    ...
    sdlog.debug(
        "SDPROXMT-003",
        "{} search-API queries to process (max_thread_per_host={},timeout={})".format(
            total_query_to_process,
            max_thread_per_host,
            Preferences().api_esgf_search_http_timeout,
        ),
    )
...
if __name__ == '__main__':
    url="http://%s/esg-search/search?fields=*&realm=atmos&project=CMIP5&time_frequency=mon&experiment=rcp26&variable=tasmin&model=CNRM-CM5&model=CSIRO-Mk3-6-0&model=BCC-CSM1-1-m&ensemble=r1i1p1&type=File" % Internal().api_esgf_search_domain_name
