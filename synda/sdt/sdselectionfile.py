...
from synda.source.config.process.download.dataset.selection.constants import STRUCTURE as SELECTION_STRUCTURE
...
def read_selection(filename):
    ...
    path="%s/%s"%(selection_folder,filename)
...
def add_selection(us):
    ...
    if not exists_selection(us):
        ...
        us.set_status(SELECTION_STRUCTURE["status"]["new"])
        ...
        if l__file_checksum==from_db_us.get_checksum():
            ...
            if us.get_status()==SELECTION_STRUCTURE["status"]["normal"]:

                # nothing to do here (let (a) and (b) decide if we need fullscan)
                pass

            elif us.get_status()==SELECTION_STRUCTURE["status"]["modified"]:

                us.set_fullscan(True)

            elif us.get_status()==SELECTION_STRUCTURE["status"]["new"]:
            ...
            us.set_status(SELECTION_STRUCTURE["status"]["modified"]) # update status
...
def reset():
    ...
    for filename in get_selection_file_list():
        us=Selection(filename=filename,logger=get_logger())
        us.set_status(SELECTION_STRUCTURE["status"]["normal"])
