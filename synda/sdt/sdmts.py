...
class DatabaseStorage(Storage):
    ...
    def get_chunks_GENERATOR(self):
        ...
        """
        processes_chunksize = Internal().processes_chunksize
        with contextlib.closing(self.conn.cursor()) as c:
            c.execute("select %s from data"%columns)
            while True:
                results = c.fetchmany(processes_chunksize)
                if not results:
                    break

                # WARNING: two (processes_chunksize) list in memory at the same time
                li=[]
                for rs in results:
...
def get_uniq_fullpath_db_filename():
    ...
    db_folder = TreePath().get("db")
    dbfile=os.path.join(db_folder,dbfilename)
    return dbfile
