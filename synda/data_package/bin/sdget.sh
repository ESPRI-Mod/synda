...
##################################
#  @program        synda
#  @description    climate models data_package transfer program
...
# 20 => 403 Forbidden
#       (permission denied after the redirect to ORP, means user do not have permission to access data_package)
...
if [ $parse_output -eq 1 ]; then
    ...
    #     - but if "--tries" is set to 1, HPSS data_package access doesn't work
