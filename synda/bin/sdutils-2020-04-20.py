...
def compute_checksum( file_fullpath, checksum_type=sdconst.CHECKSUM_TYPE_MD5, blocksize=(1024*64),
                      newtest=False ):
    # 2019.08.01: no promising tweaks left; leave the code here for a few days nevertheless
    ...
    if newtest==False:
        with open(file_fullpath, mode='rb') as f:
            ...
            for buf in iter(partial(f.read, blocksize), b''):
                d.update(buf)
        return d.hexdigest()
    else:   # various experimental tweaks in this section:
        blocksize=blocksize*4
        #with open(file_fullpath, mode='rb', buffering=0) as f:
        with open(file_fullpath, mode='rb') as f:
