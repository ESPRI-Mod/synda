...
figs = make_subplots(
    ...
    subplot_titles=(
        ...
        'synda new version - code',
        'synda new version - db',
    ),
    specs=[
...
figs.add_trace(
    go.Scatter(
        x=x,
        y=y,
        name='synda current version - observations',
...
x2 = asyncio_aiohttp["size"]
y2 = asyncio_aiohttp["duration"]
...
titles = (
 ...
 'synda new version - code ({} s)'.format(asyncio_aiohttp_duration["duration"]),
 'synda new version - db ({} s)'.format(asyncio_aiohttp_db_duration["duration"]),
)
