...
checksum = '79e92073e7f0e31a968744ef4c0cbeb80104a07bb9ba82862bab2ee50212bd8a'


@pytest.mark.asyncio
async def test_set_next_url_ko():
    ...
    new_task.set_fallback_strategy(next_url_on_error=True)
    assert url == new_task.get_file_instance().url

    assert new_task.get_file_instance().status == TRANSFER["status"]['error']
    assert new_task.get_file_instance().priority == 999
    assert new_task.get_file_instance().error_msg == DOWNLOAD_ERROR_MSG
