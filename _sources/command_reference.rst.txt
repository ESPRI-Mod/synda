...
.. code-block:: bash
    ...
    usage: synda certificate [-h] [-d] [-o OPENID] [-p PASSWORD] [-x] [{renew,print}]
    ...
    description
      dataset_version
        ...
        - For each type of error,
          ...
          - the number of datasets to which it applies, ie datasets with at least one version having in error.
      file_variable
...
.. code-block:: bash
    ...
    optional arguments:
      ...
      -i INDEX_HOST, --index_host INDEX_HOST retrieves parameters from the specified index
...
Download management, previously known as daemon management in older synda versions.
...
get
******
Direct download of a selection file or specific dataset
...
Helper subcommand to initialize the required folders and files required by synda
.. code-block:: bash
...
Adds a dataset to the download queue
...
Prints out an introduction to synda commands
...
Displays performance and disk usage metrics
...
Prints the facets harvested from ESGF nodes
...
Displays download queue progress
...
Moves the download to next replica
...
Removes all download tasks in 'waiting' and 'error' status
.. code-block:: bash
...
Retries a failed or waiting transfer (literally switches status from error to waiting in queue db)
...
Performs a search against ESGF for one or many datasets
...
Lists available selection files
...
Displays detailed information about dataset
...
.. code-block:: bash
    ...
    optional arguments:
      ...
      -n, --no_default      prevent loading default value
      -z, --dry_run
      -i, --incremental     Limit action on files which appeared since last run (experimental)
...
Updates ESGF parameters local cache
...
Runs 'install' command on all selection files
...
Displays running transfer(s) information
