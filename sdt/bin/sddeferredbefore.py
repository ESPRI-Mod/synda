...
#                             All Rights Reserved”
#  @svn_file       $Id: sddeferredbefore.py 12605 2014-03-18 07:31:36Z jerome $
#  @version        $Rev: 12638 $
...
def add_default_parameter(o,name,value):
    if sdstream.is_stream(o):
        ...
        k="%s%s"%(sdconst.BIDPP,name)
        for facets_group in o:
            facets_group[k]=value
    else:
        # assume parameter

        o.append("%s%s=%s"%(sdconst.BIDPP,name,value))
...
def add_forced_parameter(o,name,value):
    if sdstream.is_stream(o):
        ...
        k="%s%s"%(sdconst.BIFPP,name)
        for facets_group in o:
            facets_group[k]=value
    else:
        # assume parameter

        o.append("%s%s=%s"%(sdconst.BIFPP,name,value))
