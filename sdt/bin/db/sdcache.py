...
from sdt.bin.commons.esgf import sdproxy_ra
...
def run(host=None, reload=False, project=None):
    ...
    with session.create():
        ...
        if reload:
            sdlog.info("SDDCACHE-001", "Reloading parameters (index={})".format(host))
            _reload_parameters(parameters)
        else:
            sdlog.info("SDDCACHE-002", "Updating parameters (index=%s)".format(host))
        ...
        for i, item in enumerate(pvalues):
            try:
                if item is None:
                    ...
                    dao.add_parameter(name=pname, value=item.name)
        ...
        if len(pvalues) == 0:
            ...
            if not dao.exists_parameter_name(pname):
                ...
                try:
                    ...
                    sdlog.warning('SDDCACHE-003', 'Value has duplicate in the db. Warn your datanode manager')
        elif len(pvalues) > 1:
