...
def run(host, port, username, force_renew_certificate=False, force_renew_ca_certificates=False, password=None):
    ...
    if not certificate_exists():
        ...
        os.chmod(sdconfig.esgf_x509_proxy, 0o600)  # needed by globus-url-copy
...
def certificate_is_valid():
    ...
    if os.path.isfile(sdconfig.esgf_x509_proxy):
        with open(sdconfig.esgf_x509_proxy, 'rb') as pem_file:
            ...
            print((cert.not_valid_after - datetime.now()).total_seconds())
            if (cert.not_valid_after - datetime.now()).total_seconds() > 500:
                return True
            else:
                return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--force_renew_certificate', '-r', action='store_true',
