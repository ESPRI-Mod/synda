...
def remove_malformed_dataset_functional_id(files):
    ...
    for f in files:
        ...
        if m != None:
            ...
            sdlog.warning("SDPREPAR-002", "Incorrect dataset_functional_id ('%s')" % (f["dataset_functional_id"],),
                          stderr=False)
            reject.append(f)
...
def remove_malformed_dataset_version(files):
    ...
    for f in files:
        if sdidtest.is_version_number(f["dataset_version"]):
            ...
            sdlog.warning("SDPREPAR-003", "Incorrect dataset version ('%s')" % (f["dataset_functional_id"],),
                          stderr=False)
            reject.append(f)
