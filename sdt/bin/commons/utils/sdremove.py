...
from sdt.bin.commons.utils.sdtools import print_stderr, print_stdout
...
from sdt.bin.commons.utils import sddelete
from sdt.bin.commons.utils import sddeletefile
from sdt.bin.commons.utils import sddeletedataset
from sdt.bin.commons.utils import sdcleanup

from sdt.bin.models import sdtypes
from sdt.bin.commons.param import sdmdcommon
...
def run_local(args, stream):
    ...
    try:
        ...
        if args.verbose:
            for f in files:
                buf = "file_id=%d, status=%s, local_path=%s, url=%s" % (f.file_id,
                                                                        f.status,
