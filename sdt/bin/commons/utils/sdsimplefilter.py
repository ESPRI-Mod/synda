...
def run(metadata, filter_name, filter_value, mode):
    if mode == 'keep':
        ...
        raise SDException("SDSIMPLF-002", "Incorrect mode ({})".format(mode))
...
def keep_matching_files(files, filter_name, filter_value):
    ...
    for f in files:
        if filter_name not in f:
            raise SDException("SDSIMPLF-001",
                              "Filter name not found in file attributes (filter_name={})".format(filter_name))
...
def remove_matching_files(files, filter_name, filter_value):
    ...
    for f in files:
        if filter_name not in f:
            raise SDException("SDSIMPLF-003",
                              "Filter name not found in file attributes (filter_name={})".format(filter_name))
        if f[filter_name] != filter_value:
            new_files.append(f)
    files = new_files
...
def remove_matching_files_substr(files, filter_name, filter_value):
    ...
    for f in files:
        if filter_name not in f:
            raise SDException("SDSIMPLF-004",
                              "Filter name not found in file attributes (filter_name={})".format(filter_name))
