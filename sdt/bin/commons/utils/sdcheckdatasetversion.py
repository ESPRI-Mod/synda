...
from sdt.bin.models import sddatasetversion
from sdt.bin.commons.utils import sdtxt2pdf, sddump
...
def run(args):
    ...
    all_dsv = sddump.dump_ESGF(parameter=args.parameter, selection_file=args.selection_file,
                               fields='master_id,version,timestamp', dry_run=args.dry_run, record=args.record,
    ...
    versatile_print('  %*d have a timestamp field on some of their versions' % (
        digits, sum(map(lambda x: 1 if x['flags'] == 3 else 0, dsv_grouped_by_master_id.values()))))
    versatile_print('  %*d have a timestamp field on none of their versions' % (
        digits, sum(map(lambda x: 1 if x['flags'] == 2 else 0, dsv_grouped_by_master_id.values()))))
