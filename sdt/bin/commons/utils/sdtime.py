...
def compute_time_delta(start_date, end_date):
    ...
    #
    seconds = (delt.microseconds + (
            delt.seconds + delt.days * 24 * 3600) * 10 ** 6) / 10 ** 6  # microsecond is present here, but disappear after the last division
...
def now():
    ...
    while True:
        ...
        l__now = datetime.datetime.now().isoformat(
            " ")  # isoformat returns a string representing the date and time in ISO 8601 format, YYYY-MM-DDTHH:MM:SS.mmmmmm, or, if microsecond is 0, YYYY-MM-DDTHH:MM:SS

        if len(
                l__now) == 26:  # with microseconds, length is 26. Doing this, we ensure we don't return timestamp without microsecond (see note in methods header)
            return l__now
...
class SDTimer():
    ...
    def get_elapsed_time(cls, start_time):
        ...
        return (delt.microseconds + (
                delt.seconds + delt.days * 24 * 3600) * 10 ** 6) / 10 ** 6  # microsecond is present here, but disappear after the last division. duration unit is second.
