...
from sdt.bin.commons.utils.sdexception import SDException, FatalException
...
def transfers_end():
    ...
    for task_id in globus_tasks:
        ...
        sdlog.debug("SDDMGLOB-016",
                    "Checking the status of Globus transfer tasks, id: %s, status: %s" % (task_id, status))
        for item in globus_tasks[task_id]['items']:
            ...
            if status == "SUCCEEDED":
                ...
                if int(tr.size) != os.path.getsize(tr.get_full_local_path()):
                    sdlog.error("SDDMGLOB-002", "size don't match (remote_size=%i,local_size=%i,local_path=%s)" % (
                ...
                if tr.status == sdconst.TRANSFER_STATUS_DONE:
                    ...
                    tr.error_msg = ""
                    sdlog.info("SDDMGLOB-101", "Transfer done ({})".format(str(tr)))
                ...
                sdlog.info("SDDMGLOB-101", "Transfer failed ({})".format(str(tr)))
...
def transfers_begin(transfers):
    ...
    for tr in transfers:
        ...
        sdlog.info("SDDMGLOB-001",
                   "src_endpoint: {}, src_path: {}, local_path: {}".format(src_endpoint, src_path, local_path))
    ...
    for src_endpoint in globus_transfers:
        ...
        sdlog.info("SDDMGLOB-004",
                   "Globus transfer, source endpoint: {}, destination endpoint: {}".format(src_endpoint, dst_endpoint))
        for item in globus_transfers[src_endpoint]['items']:
            t.add_item(item['src_path'], item['dst_path'])
            sdlog.info("SDDMGLOB-005", "Globus transfer item, source path: {}, destination path: {}".format(
        ...
        if code != 202:
            sdlog.error("SDDMGLOB-006", "Error: Cannot add a transfer: ({}, {})".format(code, message))
            raise FatalException()
        task_id = data['task_id']
        sdlog.info("SDDMGLOB-007", "Submitted Globus task, id: {}".format(task_id))
...
def map_to_globus(url):
    ...
    sdlog.debug("SDDMGLOB-024", "Mapped url {} to {}{}".format(url, src_endpoint, src_path))
...
def activate_endpoint(api, ep=None):
    ...
    try:
        ...
        sdlog.error("SDDMGLOB-028", "Error: Cannot activate the source endpoint: ({})".format(str(e)))
        raise FatalException()
        ...
        if self.filepath:  # only if endpoints file exists
            ...
            if force or modtime > self.modtime:
                ...
                for endpoint in root.findall("{%s}endpoint" % NS):
                    ...
                    sdlog.debug("SDDMGLOB-018",
                                "Using Globus endpoint %s : %s (%s --> %s)" % (gridftp, name, path_out, path_in))
