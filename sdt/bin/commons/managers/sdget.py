...
# TODO Get rid of the globus-url-copy external script call
# Should be rewritten in vanilla python
# Remove the methods that parse the script results.
# sdconst.HTTP_CLIENT_URLLIB -> HTTP_CLIENT
    ...
    if transfer_protocol == sdconst.TRANSFER_PROTOCOL_HTTP:
        if sdconfig.http_client == sdconst.HTTP_CLIENT_WGET:
            if status in (7, 29):
                ...
                return False
        elif sdconfig.http_client == sdconst.HTTP_CLIENT:
            return False
        else:
            assert False
    elif transfer_protocol == sdconst.TRANSFER_PROTOCOL_GRIDFTP:
...
def prepare_args(url, full_local_path, script, debug, timeout, verbosity, hpss):
    li = [script, '-l', sdconfig.log_folder, '-T', sdconfig.tmp_folder, '-t', str(timeout), '-c',
          sdconfig.get_security_dir(), url, full_local_path]
...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        if status != 0:
            if not args.quiet:
                print_stderr('Download failed: {}'.format(url))
