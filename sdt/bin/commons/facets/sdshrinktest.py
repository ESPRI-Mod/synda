...
def is_nearestpost_enabled(metadata):
    ...
    if sdconfig.nearest_schedule == 'post' and nearest_flag_set_on_all_files(metadata):
        ...
        result = False
    sdlog.debug("SSHRINKT-002", "nearestpost is {}".format(result))
    return result


def nearest_flag_set_on_all_files(metadata):
    """
    This func checks that all files have the 'nearest' flag
    (as sdnearestpost processing type is 'interfile', we need ALL files to be flagged).
    """
    status = True
    ...
    light_metadata = sdlmattrfilter.run(metadata, ['attached_parameters'])
