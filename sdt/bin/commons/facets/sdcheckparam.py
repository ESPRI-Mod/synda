...
"""
from sdt.bin.commons.utils import sdlog, sdi18n
...
def check_replica_not_set_when_using_nearestpost(facets_groups):
    """
    Do not continue if 'replica' has been set
    ('replica' must not be set, as we want to search for the nearest in all existing copies of the file).
    """
    ...
    for facets_group in facets_groups:
        for name in sdconst.SASP:
            if name in facets_group:
                ...
                if len(value) > 1:
                    raise sdexception.SDException("SYDCHECK-004", "Too much values for '%s' parameter (value='%s')" % (
                        name, str(value)))
    ...
    for facets_group in facets_groups:
        for name, values in sddquery.search_api_parameters(facets_group).iteritems():
            if not sdparam.exists_parameter_name(name):
                ...
                raise sdexception.UnknownParameterNameException("SYDCHECK-008",
                                                                "Unknown parameter name: {}".format(name))
...
def check_parameter_values(name, values):
    for value in values:
        if not sdparam.exists_parameter_value(name, value):
            ...
            msg = "Unknown value '{}' for '{}' parameter. {}".format(value, name, sdi18n.m0021)

            if sdconfig.unknown_value_behaviour == 'warning':
                sdprint.print_stderr('WARNING: ' + msg)
            else:
