...
"""This module detects frozen wget."""
# TODO is this still required after wget bites the dust.
        ...
        try:
            ...
            if len(p.cmdline()) > 0:
                ...
                if cmd == "wget":
                    ...
                    # check array size not to raise exception in next line if wget is not related to our application
                    if len(parent.cmdline()) > 1:
                        m = re.search("sdget.sh", parent.cmdline()[1])
                        if m is not None:
                            # retrieve wget args
                            local_path = p.cmdline()[
                                2]  # this is a huge hack for the checksum/FIFO case !!!! (this class need to know the local file associated with the process, but because of the FIFO, this dest file do not show in "ps fax" output, so we put the dest file in unused " -D domain-list" option (this option is used only in recursive mode, which we do not use))
                            ...
                            if debug:
                                ...
                                if os.path.isfile(local_path):
                                    ...
                                    # this can occur if wget already started, but didn't create the output file yet
                                    sdlog.info("SDWATCHD-274", "file not found (%s%s)" % (local_path,
                                                                                          pid))
