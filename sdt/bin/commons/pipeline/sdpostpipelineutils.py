...
# the two methods below is to have some abstraction over file type
...
def get_functional_identifier_value(f):
    ...
    if name in f:
        ...
        raise SDException('SYDUTILS-020', 'Incorrect identifier ({})'.format(name))
...
def get_functional_identifier_name(f):
    if f["type"] == sdconst.SA_TYPE_FILE:
        ...
        raise SDException('SYDUTILS-028', 'Incorrect type ({})'.format(f["type"]))
