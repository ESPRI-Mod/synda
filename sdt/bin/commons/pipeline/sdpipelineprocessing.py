...
def run_pipeline(metadata, po, io_mode=sdconst.PROCESSING_FETCH_MODE_GENERATOR):
    ...
    sdlog.debug("SYNDPIPR-001", "Start chunk loop (files-count={})".format(metadata.count()))
    ...
    sdlog.debug("SYNDPIPR-003", "Chunk loop completed (files-count={})".format(metadata.count()))
