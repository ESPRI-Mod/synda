...
def get_one_file(host=sdindex.get_default_index(), project=None, query=None, dry_run=None):
    ...
    project_filter = '' if project is None else "&project={}".format(project)
    query_filter = '' if query is None else "&query={}".format(query)

    fmt = sdurlutils.get_solr_output_format(sdconfig.searchapi_output_format)

    url = 'http://{}/esg-search/search?type=File{}{}&fields=*&format={}'.format(host, project_filter, query_filter, fmt)
    if dry_run:
        print(url)
        return None
    else:
