...
def get_scalar(stream, name, default=None, type_=None):
    ...
    dquery = stream[
        0]  # note that stream can have more than one dquery inside, but then we assume all dquery have same value for that parameter.
...
def is_stream(o):
    ...
    if isinstance(o[0],
                  dict):  # This test is based on the fact that stream contains only dict and parameter contains only str)
