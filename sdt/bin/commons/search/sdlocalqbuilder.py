...
Note
    'sdlocalqbuilder' stands for 'SynDa local query builder'
"""
...
def run(facets_groups):
    ...
    for dquery in facets_groups:
        ...
        if type_ == sdconst.SA_TYPE_FILE:
            ...
            raise SDException('SDLOQUBU-001', 'Incorrect type ({})'.format(type_))

        # Add limit clause
        sqlquery += limit_clause
        query = {'sqlquery': sqlquery, 'attached_parameters': {'type': dquery['type']}}
...
def build_file_query(facets):
    ...
    if 'local_path' in facets:
        ...
        value = facets['query'][
            0]  # 'query' is always scalar (space and comma characters can't be used for now in this filter)
        buf = "'%%%s%%'" % value
        q = "select * from file where file_functional_id like %s order by file_functional_id desc" % (buf,)
    else:
        # mainstream search

        di = dict((k, facets[k]) for k in facets if
                  k in ['insertion_group_id', 'status', 'error_msg', 'sdget_status', 'data_node', 'model', 'project',
                        'variable', 'filename'])  # keep only local compatible keys
        if len(di) < 1:
            q = "SELECT * FROM file"
        else:
...
def build_dataset_query(facets):
    ...
    if 'local_path' in facets:
        ...
        q = "select * from dataset where %s" % " OR ".join(
            ["local_path like '%%%s%%'" % sdproduct.replace_product_with_sql_wildcard(lp) for lp in
             facets['local_path']])
        ...
        di = dict(
            (k, facets[k]) for k in facets if k in ['status', 'model', 'project'])  # keep only local compatible keys
        if len(di) < 1:
...
print(build_file_query({'local_path': '/output1/'}))
