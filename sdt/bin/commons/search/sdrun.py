...
from sdt.bin.commons.search import sdproxy_mt
...
def run(queries, parallel=True):
    if parallel:
        ...
        (queries_with_index_host, queries_without_index_host) = split_queries(
            queries)  # we need this, because query with specific index host can't be parallelized
...
def sequential_exec(queries):
    ...
    for i, q in enumerate(queries):
        sdlog.info("SYNDARUN-001", "Process query {}".format(i))
