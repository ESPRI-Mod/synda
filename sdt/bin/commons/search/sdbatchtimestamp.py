...
from sdt.bin.commons.utils.sdexception import MissingDatasetTimestampUrlException, MissingTimestampException
...
def run(squeries, metadata, parallel):
    ...
    sdlog.info("SYNDABTI-100", "{} datasets with timestamp retrieved".format(len(datasets_timestamps)))
...
def add_dataset_timestamp(files, datasets_timestamps):
    ...
    for dataset_functional_id in dataset_with_missing_timestamp:
        sdlog.info("SYNDABTI-200", "dataset timestamp not found ({})".format(dataset_functional_id))
...
def get_datasets_timestamps(squeries, parallel):
    ...
    for d in metadata.get_files():  # warning: load list in memory
        ...
        try:
            ...
            sdlog.info("SYNDABTI-500", "dataset found but dataset timestamp is missing ({})".format(instance_id))
