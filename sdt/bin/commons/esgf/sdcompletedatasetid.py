...
def run(facets_groups):
    for facets_group in facets_groups:
        if 'dataset_id' in facets_group:
            # This is normal: dataset_id IS a list of values
            # (MEMO: this module is used to build the query (i.e. not used after the search-api call))
            dataset_id_list = facets_group['dataset_id']
            new_values = []
            ...
            for dataset_id in dataset_id_list:
                if is_data_node_present_in_dataset_id(dataset_id):
                    ...
                    # meaningful as instance_id is dataset_id without data_node
...
def instance_id_to_dataset_id(instance_id, facets_group):
    if "data_node" in facets_group:
        ...
        dnode = li[0]
        return "{}|{}".format(instance_id, dnode)
    else:
...
def retrieve_full_dataset_id(instance_id, replica):
    ...
    if len(datanodes) < 1:
        ...
        return "{}|{}".format(instance_id, data_node)


def get_data_nodes(instance_id, replica):
    """Return one or more data_nodes depending on the 'replica' flag."""

    parameter = ['limit=50', 'type=Dataset', 'instance_id={}'.format(instance_id)]

    if replica is not None:
        parameter.append('replica={}'.format(replica))
