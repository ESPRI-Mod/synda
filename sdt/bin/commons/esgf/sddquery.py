...
Notes
    - sddquery stands for 'SynDa Deserialized query'
...
def print_(dquery):
    # we cast here as verbose can be str (set from parameter) or bool (set from '-v' option)
    verbose = get_scalar(dquery, 'verbose', default=False, type_=bool)
    if verbose:
        print(dquery)
    else:
        print(dquery['url'])
...
def set_scalar(dquery, name, value):
    if isinstance(value, list):
        raise SDException("SDDQUERY-003", "Incorrect type (%s)".format(name, ))

    if name in dquery:
        raise SDException("SDDQUERY-002", "Key already exist (%s)".format(name, ))
...
def get_scalar(facets_group, name, default=None, type_=None):
    ...
    if name in facets_group:
        ...
        if isinstance(value, list):
            ...
            if len(value) > 1:
                raise SDException("SDDQUERY-001",
                                  "Too much values for '{}' parameter (value='{}')".format(name, str(value)))
            v = value[0]
        else:
            v = value
