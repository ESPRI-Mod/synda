...
def file_full_search(args, stream=None):
    ...
    if stream is None:
        stream = get_stream(subcommand=args.subcommand, parameter=args.parameters, selection_file=args.selection_file,
                            no_default=args.no_default, raise_exception_if_empty=True)
    ...
    if args.subcommand in ('install', 'stat'):  # prevent incremental mode for 'remove' subcommand
        ...
        if args.timestamp_left_boundary and args.incremental:
            raise sdexception.SDException('SYNUTILS-003',
