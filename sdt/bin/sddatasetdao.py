...
def add_dataset(dataset, commit=True, conn=sddb.conn):
    keys_to_insert = ['local_path', 'path', 'path_without_version', 'dataset_functional_id', 'template', 'version',
                      'status', 'latest', 'crea_date', 'last_mod_date', 'project', 'model', 'timestamp']
    return sdsqlutils.insert(dataset, keys_to_insert, commit, conn)


def get_dataset_by_local_path(local_path, not_found_raise_exception=False):
    datasets = get_datasets(local_path)
...
def get_dataset(path=None, dataset_id=None, dataset_functional_id=None, conn=sddb.conn):
    ...
    for va in (path, dataset_id, dataset_functional_id):
        if va is not None:
            count = count + 1
    if count > 1:
        raise SDException("SYNCDDAO-123", "Too much arguments (path=%s,dataset_id=%s,dataset_functional_id=%s)" % (
            path, dataset_id, dataset_functional_id,))
...
def update_dataset(d, commit=True, conn=sddb.conn,
                   keys=['latest', 'status', 'last_mod_date', 'latest_date', 'last_done_transfer_date']):
...
def get_datasets(limit=None, conn=sddb.conn, **search_constraints):  # don't change arguments order here
    ...
    if len(search_constraints) > 0:
        q = "select * from dataset where %s order by path asc %s" % (
            sdsqlutils.build_search_placeholder(search_constraints), limit_clause)
...
if __name__ == '__main__':
    ...
    if args.dataset is not None:
        ...
        d = get_dataset_by_local_path(local_path=args.dataset)
        print(d)
