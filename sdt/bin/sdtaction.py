...
def facet(args):
    ...
    if sdparam.exists_parameter_name(
            args.facet_name):  # first, we check in cache so to quickly return if facet is unknown
...
def get(args):
    ...
    if len(urls) == 0:
        ...
        files = sdrfile.get_files(stream=stream, post_pipeline_mode='file',
                                  dry_run=args.dry_run)  # yes: this is the second time we run sdinference filter, but it doesn't hurt as sdinference is idempotent
                    ...
                    if d['status'] == sdconst.DATASET_STATUS_COMPLETE:
                        ...
                        for v in d['variable']:
                            if v in facets_group[
                                ...
                                # hack
                                if sddomainutils.is_one_var_per_ds(d[
